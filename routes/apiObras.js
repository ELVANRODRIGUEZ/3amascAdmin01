...
router.get('/', async (req, res) => {
    ...
    try {
        console.log('Request received for /api/obras');
        const client = await pool.connect(); // The function will pause here until we have a connection 
        // result.
        console.log('Connected to the database to SELECT OBRAS');
        
        const result = await client.query('SELECT * FROM obras'); // We query the connection object (wich 
        // connects to the database) by using the '.query' method, which passes on the SQL command.
        const obras = result.rows; // The query result object stores the fetched rows, and they can be
        ...
        console.log('Released database connection to SELECT OBRAS');
        
        res.json(obras);// Sending the response in JSON format
        console.log('Sent "obras" data as JSON');
        
        // console.log(obras); // Verification Log.
...
// New endpoint for updating 'obras'
router.put('/:id', async (req, res) => {
    // The following code lines are called 'object destructuring in Javascript'.
    // It will extract properties (the ones indicated inside "{}") from an object,
    // in this case, the 'req' object, which is the one sent on the PUT request.
    const { id } = req.params;
    const { obra } = req.body; // Assuming you pass the new name and description in the request body

    // To manage errors properly, everything we do in this function we are going to try it in 
    // order to catch any possible error.
    try {

        console.log(`Request received for PUT /api/obras/${id}`);
        const client = await pool.connect();// The function will pause here until we have a connection 
        // result.
        console.log('Connected to the database to UPDATE OBRAS');

        // We query the connection object (wich connects to the database) by using the '.query' method, 
        // which passes on the SQL command.
        const result = await client.query('UPDATE obras SET obra = $1 WHERE id = $2 RETURNING *', [obra, id]);
        const updatedObra = result.rows[0]; // The query result object stores the fetched rows, and they can be
        // called with the '.rows' property.
        ...
        console.log('Released database connection to UPDATE OBRAS');

        res.json(updatedObra);// Sending the response in JSON format
        console.log('Sent updated obra as JSON');
