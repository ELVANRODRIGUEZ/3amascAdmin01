...
router.get('/', async (req, res) => {
  ...
  try {
      console.log('Request received for /api/unidades');
      const client = await pool.connect(); // The function will pause here until we have a connection 
      // result.
      console.log('Connected to the database to SELECT UNIDADES');
      
      const result = await client.query('SELECT * FROM unidades'); // We query the connection object (wich 
      // connects to the database) by using the '.query' method, which passes on the SQL command.
      const unidades = result.rows; // The query result object stores the fetched rows, and they can be
      ...
      console.log('Released database connection to SELECT UNIDADES');
      
      res.json(unidades);// Sending the response in JSON format
      console.log('Sent "unidades" data as JSON');
      
      // console.log(unidades); // Verification Log.
